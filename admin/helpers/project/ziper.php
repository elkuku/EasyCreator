...
class EcrProjectZiper extends JObject
{
	...
	private function initBuildOpts($buildopts)
	{
		...
		, 'createMD5', 'createMD5Compressed', 'createVersionSubdir');
	...
	private function createArchive()
	{
		...
		{
			$zipDir = $this->build_dir.($this->buildopts['createVersionSubdir'] ? DS.$this->project->version : '');
		}
		else
		{
			$zipDir = $this->build_dir.DS.$this->project->comName.($this->buildopts['createVersionSubdir'] ? DS.$this->project->version : '');
		}
		...
		if(0 === strpos($this->project->getZipPath(), ECRPATH_BUILDS))
		{
			$hrefBase = JURI::root().str_replace(JPATH_ROOT, '', ECRPATH_BUILDS)
				.'/'.$this->project->comName . ($this->buildopts['createVersionSubdir'] ? '/'.$this->project->version : '');
		...
		{
			$hrefBase = 'file://'.$this->project->getZipPath().($this->buildopts['createVersionSubdir'] ? DIRECTORY_SEPARATOR.$this->project->version : '');
		}
